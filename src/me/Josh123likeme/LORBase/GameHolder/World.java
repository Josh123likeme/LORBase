...
public class World {
	...
	public World(Save save, int level) {
		...
		for (int y = 0; y < chunks.length; y++) {
			...
			Main.game.debugInfo.addOrUpdateTask("Stitching Chunks", 
					Math.round((double) y / chunks.length * 100) + "%"); //loading info
			
			for (int x = 0; x < chunks[0].length; x++) {
				
				chunks[y][x].stitchTexture();
				
			}
			
		}
		
		Main.game.debugInfo.removeTask("Stitching Chunks"); //loading info
