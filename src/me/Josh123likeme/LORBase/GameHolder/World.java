...
import me.Josh123likeme.LORBase.ItemHolder.TELEPORTATION_CORE;
...
public class World {
	...
	public Vector2D mouseWorldPos = new Vector2D();
	...
	public World(Save save, int level) {
		...
		//add zombies
		int numberOfZombies = (((80 + 16 * level) * (80 + 16 * level)) / 500) * level;
		...
		Main.game.debugInfo.removeTask("Spawning Zombies"); //loading info
		...
		Main.game.debugInfo.removeTask("Placing Gargoyles"); //loading info
	...
	public void update() {
		...
		if (mouseWitness.isRightClicked()) {
			...
			if (getWall((int) mouseWorldPos.X, (int) mouseWorldPos.Y) != Wall.AIR) canTeleport = false;
			if (getWall((int) (mouseWorldPos.X + player.getSize()), (int) mouseWorldPos.Y) != Wall.AIR) canTeleport = false;
			if (getWall((int) mouseWorldPos.X, (int) (mouseWorldPos.Y + player.getSize())) != Wall.AIR) canTeleport = false;
			if (getWall((int) (mouseWorldPos.X + player.getSize()), (int) (mouseWorldPos.Y + player.getSize())) != Wall.AIR) canTeleport = false;
			...
			if (canTeleport) {
				...
				for (int i = 0; i < items.length; i++) {
					...
					//if player has teleportation core, they can teleport. we then remove the teleportation core
					if (items[i] != null && items[i].getClass().getSimpleName().equals(TELEPORTATION_CORE.class.getSimpleName())) {
		...
		Main.game.debugInfo.addOrUpdateTask("Entity update time: ");
		
		updateEntities();
		
		Main.game.debugInfo.addOrUpdateTask("Entity update time: ", (double) (System.nanoTime() - start) / 1000000 + " ms");
		
		//ui updates
		if (Main.game.keyboardWitness.isButtonTyped(ButtonType.INVENTORY)) {
			
			if (inventoryOpen) player.getInventory().closeInventory();
	...
	private void updateEntities() {
		...
		for (Entity entity : entities) {
			...
			if (entity.getPosition().distanceTo(player.getPosition()) < 50) entity.update(this);
