...
public class GARGOYLE extends Entity implements IAttack, IChase, IMoveable, IHealthy, ISleep {
	...
	public void kill() {
		...
		double numberSeed = random.nextDouble();
		
		int numberOfDrops = (int) Math.ceil(5 * numberSeed * numberSeed);
		
		for (int i = 0; i < numberOfDrops; i++) {
			
			Item drop = new TELEPORTATION_CORE();
			
			world.addEntity(new ITEM_ENTITY(world, pos.clone().fluctuate(0.5d), 270, drop, 0.2d));
